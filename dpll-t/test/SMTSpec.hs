...
rubric = do
  criterion "dpllT" 1 . passOrFail $ do
    it "solves satisfiable constraints without blocking" $ do
      check "x>=1 & y>=1" @?= True
      check "(x>=1 | y>=1)" @?= True
      check "x=1 | y=2" @?= True
    ...
    it "solves satisfiable constraints with blocking" $ do
    ...
    it "solves complex constraints (may take a bit of time)" $ do
