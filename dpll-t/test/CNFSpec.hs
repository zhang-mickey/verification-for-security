...
rubric = do
  ...
  criterion "Transform" (9/10) $ do
    criterion "distribute" (1/2) . passOrFail $ do
      let distribute' lhs rhs = set $ CNF.distribute lhs rhs
      it "correctly computes the base case (no conjuncts)" $ do
        distribute' (cnf "x") (cnf "!y") @?= cnf' "x | !y"
        let lhs = cnf "a | b | c"
        let rhs = cnf "d | e | f"
        distribute' lhs rhs  @?= cnf' "a | b | c | d | e | f"

      it "correctly distributes singles" $ do
        let a  = cnf "a"
        let bcd = cnf "b & c & d"
        distribute' a bcd @?= cnf' "(a | b) & (a | c) & (a | d)"
        distribute' bcd a @?= cnf' "(b | a) & (c | a) & (d | a)"

      it "correctly recursively distributes" $ do
        let lhs0 = cnf "a & b"
        let rhs0 = cnf "c & d"
        let answer0 = cnf' "(a | c) & (a | d) & (b | c) & (b | d)"
        distribute' lhs0 rhs0 @?= answer0

        let lhs1 = cnf "a & b & c"
        let rhs1 = cnf "d & (e | f)"
        let answer1 = cnf' "(a | d) & (a | e | f) & (b | d) & (b | e | f) & (c | d) & (c | e | f)"
        distribute' lhs1 rhs1 @?= answer1

    criterion "cnf" (1/2) . passOrFail $ do
      let transform = set . CNF.cnf . prop
